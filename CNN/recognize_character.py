...
def recognize(dir):
	...
	for file in flist:
		...
		if '-1' in file:
			predictions[file] = ottakshara_mapping[keras_predict(os.path.abspath(os.path.join(dir, file)), ottakshara_model)]
		#if image in a regular character
		else:
			predictions[file] = keras_predict(os.path.abspath(os.path.join(dir, file)), char_model)
