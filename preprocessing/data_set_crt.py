...
for root,dirs,files in os.walk(rootdir,topdown=False):
	dirs.sort(key=lambda f: int(''.join(filter(str.isdigit,f))))			# Sorts the dires by number
	for name in dirs:
		...
		flist = glob.glob('*.jpg')
		Parallel(n_jobs = -1)(delayed(crop)(n) for n in flist) 				# Crops all the original image for faster processing
		Parallel(n_jobs = -1)(delayed(fixedsize)(n) for n in flist)			# Resizes all images to fixed size
		Parallel(n_jobs = -1)(delayed(denoise)(n) for n in flist)			# Removes noise from all images
		Parallel(n_jobs = -1)(delayed(contrast)(n) for n in flist)			# Increase contrast for all images
		flist1 = glob.glob('*.jpg')
		Parallel(n_jobs = -1)(delayed(ht)(n) for n in flist1)				# Augmentation: changes the aspect ratio 
		Parallel(n_jobs = -1)(delayed(rtl)(n) for n in flist1)				# Augmentation: rotates the image to the left
		Parallel(n_jobs = -1)(delayed(rtr)(n) for n in flist1)				# Augmentation:	rotates the image to the right
		flist2 = glob.glob('*.jpg')
		Parallel(n_jobs = -1)(delayed(binerize)(n) for n in flist2)			# Smoothing: first binerize to remove stray noise
		Parallel(n_jobs = -1)(delayed(blur)(n) for n in flist2)				# Smoothing: blur to smooth the pixelated edges
		Parallel(n_jobs = -1)(delayed(binerize)(n) for n in flist2)			# Smoothing: second binerize to get clear output 
		flist3 = glob.glob('*.jpg')
		Parallel(n_jobs = -1)(delayed(crop)(n) for n in flist3)				# Croping since rotated images have added padding
		Parallel(n_jobs = -1)(delayed(padding)(n) for n in flist3)			# Adding fixed padding to all images
		flist4 = glob.glob('*.jpg')		
		Parallel(n_jobs = -1)(delayed(size208)(n) for n in flist4)			# Resizing to redues line cuts 
		flist5 = glob.glob('*.jpg')			
		Parallel(n_jobs = -1)(delayed(gaussianresize)(n) for n in flist5)	# Resize to ML specification and adds gaussian blur
		Parallel(n_jobs = -1)(delayed(noise)(n) for n in flist5)			# Adds noise to all images and renames

		flist6=glob.glob('*.jpg')					# Used to prevent lose of images when over wirting with same name 
		coount=10000
		for n in flist6:
